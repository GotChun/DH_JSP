...
public class PermissonFilter implements Filter {
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		pageGradeMap.put(projectPath + "/user.do", ROLE.ROLE_USER);
		pageGradeMap.put(projectPath + "/member.do", ROLE.ROLE_MEMBER);
		pageGradeMap.put(projectPath + "/admin.do", ROLE.ROLE_ADMIN);
	}
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if(myRole==null) {
			resp.sendRedirect(req.getContextPath()+"/login.do?message=" + URLEncoder.encode("로그인이 필요한 서비스입니다","UTF-8"));
			return ;
		}
		...
		// pageRole<=myRole = 허용
		if (pageRole.ordinal() > myRole.ordinal()) { // 내 롤이 페이지 롤보다 작을 경우에는 권한이 부족 , 따라서 예외페이지로 이동
